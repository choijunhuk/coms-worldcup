...
- COMS 계정 저장: 로그인 후 만든 월드컵과 결과를 내 프로필 저장소에 저장
- 앱 내부 로그인/로그아웃: `/worldcup/` 안에서 COMS 계정으로 바로 로그인하고 로그아웃
...
이 앱은 `/worldcup/` 안의 전용 로그인 화면을 먼저 보여주며, COMS 계정으로 로그인하지 않으면 월드컵 목록, 공유 모음, 플레이 화면을 볼 수 없습니다. `coms-website` 백엔드와 같은 도메인에서 COMS 로그인 세션을 사용합니다.
...
로그인과 로그아웃은 `/worldcup/` 앱 내부에서 처리합니다. 로그인 후 프로필 저장 API를 사용할 수 없는 환경에서는 생성/플레이 중인 데이터만 localStorage에 보조 저장하고, 서버 공유 링크 발행에 실패하면 텍스트 복사 기능을 사용합니다. 공유 링크는 `https://coms.kw.ac.kr/worldcup/shared/{slug}` 형식이며 공유 목록과 상세 API도 COMS 로그인 세션이 필요합니다.
